...
PACKAGESPATH   = os.path.join(ADDONS,    'packages')
ADDOND         = os.path.join(USERDATAPATH,  'addon_data')
ADDONDATA      = os.path.join(USERDATAPATH,  'addon_data', ADDON_ID)
ADVANCED       = os.path.join(USERDATAPATH,  'advancedsettings.xml')
SOURCES        = os.path.join(USERDATAPATH,  'sources.xml')
GUISETTINGS    = os.path.join(USERDATAPATH,  'guisettings.xml')
FAVOURITES     = os.path.join(USERDATAPATH,  'favourites.xml')
FAVdest        = os.path.join(ADDONDATA, 'favs')
FAVfile        = os.path.join(FAVdest, 'favourites.xml')
PROFILES       = os.path.join(USERDATAPATH,  'profiles.xml')
...
AUTOUPDATE     = glo_var.AUTOUPDATE
WIZARDFILE     = glo_var.WIZARDFILE
#AUTOINSTALL    = glo_var.AUTOINSTALL
#REPOADDONXML   = glo_var.REPOADDONXML
#REPOZIPURL     = glo_var.REPOZIPURL
#CONTACT        = glo_var.CONTACT
COLOR1         = glo_var.COLOR1
COLOR2         = glo_var.COLOR2
COLOR3         = glo_var.COLOR3
COLOR4         = glo_var.COLOR4
...
def checkBuild(name, ret):
	...
	link = openURL(BUILDFILE).replace('\n','').replace('\r','').replace('\t','').replace('gui=""', 'gui="http://"').replace('theme=""', 'theme="http://"').replace('url2=""', 'url2="http://"').replace('url3=""', 'url3="http://"').replace('preview=""', 'preview="http://"').replace('preview="https://"', 'preview="http://"')
...
def getCacheSize():
	...
	cachelist = [
		(PROFILEADDONDATA),
		(ADDONDATA),
		(os.path.join(HOMEPATH,'cache')),
		(os.path.join(HOMEPATH,'temp')),
...
def disableProblematic(title=''):
	...
	files.append([os.path.join(HOMEPATH, 'cache', 'commoncache.db')])
	x = 0
	for item in addons:
		x += 1
	...
	for root, dirs, files in os.walk(url):
		for file in files:
			...
			if file.endswith(".xml") or file.endswith(".hash") or file.endswith("properies"):
				DP.update(perc, "[COLOR %s]Scanning: [COLOR %s]%s[/COLOR]" % (COLOR2, COLOR1, root.replace(HOMEPATH, '')), "[COLOR %s]%s[/COLOR]" % (COLOR1, file), "Please Wait[/COLOR]")
				a = open((os.path.join(root, file))).read()
				encodedpath  = urllib.quote(HOMEPATH)
				encodedpath2  = urllib.quote(HOMEPATH).replace('%3A','%3a').replace('%5C','%5c')
				b = a.replace(HOMEPATH, 'special://home/').replace(encodedpath, 'special://home/').replace(encodedpath2, 'special://home/')
					...
					if 'pass' in line:
						if not 'option="hidden"' in line:
							try:
								...
								count += 1
								log("[Hide Passwords] found in %s on %s" % (sett.replace(HOMEPATH, ''), line), xbmc.LOGDEBUG)
							except:
								pass
					...
					if 'pass' in line:
						if 'option="hidden"' in line:
							try:
								...
								f.replace(line, change)
								count += 1
								log("[Unhide Passwords] found in %s on %s" % (sett.replace(HOMEPATH, ''), line), xbmc.LOGDEBUG)
							except:
								pass
				f2 = open(sett, mode='w'); f2.write(f); f2.close()
...
def convertText(type):
	...
	if type == "builds":
		...
		if BUILDFILE == 'http://':
			DIALOG.ok(ADDONTITLE, "[COLOR %s]Error converting Text File:[/COLOR]" % COLOR2, "[COLOR %s]Build File set to 'http://' in glo_var.py[/COLOR]" % COLOR1)
			return
		if not working == True:
...
def asciiCheck(use=None, over=False):
	if use == None:
		source   = DIALOG.browse(3, '[COLOR %s]Select the folder you want to scan[/COLOR]' % COLOR2, 'files', HOMEPATH, False, False)
		if over == True:
			yes = 1
		else:
...
def backUpOptions(type, name=""):
	...
	bad_files     = [os.path.join(DATABASE, 'onechannelcache.db'),
					 ...
					 os.path.join(ADDOND, 'script.trakt', 'queue.db'),
					 os.path.join(HOMEPATH, 'cache', 'commoncache.db'),
	...
	if type == "build":
		if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Are you sure you wish to backup the current build?[/COLOR]" % COLOR2, nolabel="[B]Cancel Backup[/B]", yeslabel="[B]Backup Build[/B]"):
			...
			convertSpecial(HOMEPATH, True)
			asciiCheck(HOMEPATH, True)
			zipf = zipfile.ZipFile(zipname , 'w')
			DP.create("%s: Creating Zip" % ADDONTITLE, "Creating back up zip", "", "Please Wait...")
			for base, dirs, files in os.walk(HOMEPATH):
				dirs[:] = [d for d in dirs if d not in exclude_dirs]
				files[:] = [f for f in files if f not in exclude_files]
				for file in files:
					ITEM.append(file)
			...
			for base, dirs, files in os.walk(HOMEPATH):
				...
				for file in files:
					try:
						...
						try:
							zipf.write(fn, fn[len(HOMEPATH):], zipfile.ZIP_DEFLATED)
						except Exception, e:
					...
					if not fd in ['skin.confluence', 'skin.estuary']:
						if DIALOG.yesno(ADDONTITLE, "[COLOR %s]Would you like to add the following skin folder to the GuiFix Zip File?[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR]" % (COLOR1, fd), yeslabel="[B]Add Skin[/B]", nolabel="[B]Skip Skin[/B]"):
							for base, dirs, files in os.walk(os.path.join(ADDOND,fold)):
								...
								for file in files:
									fn = os.path.join(base, file)
									zipf.write(fn, fn[len(USERDATAPATH):], zipfile.ZIP_DEFLATED)
		...
		try:
			if not SKIN == 'skin.confluence':
				...
				if os.path.exists(ad_skin):
					if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, COLOR2, ADDONTITLE), "[COLOR %s]Would you like to go add the [COLOR %s]settings.xml[/COLOR] in [COLOR %s]/addon_data/[/COLOR] for?" % (COLOR2, COLOR1, COLOR1), "[COLOR %s]%s[/COLOR]"  % (COLOR1, SKIN), yeslabel="[B]Add Settings[/B]", nolabel="[B]Skip Settings[/B]"):
						...
						zipf.write(ad_skin, ad_skin.replace(HOMEPATH, ""), zipfile.ZIP_DEFLATED)
			if DIALOG.yesno('[COLOR %s]%s[/COLOR][COLOR %s]: Theme Backup[/COLOR]' % (COLOR1, COLOR2, ADDONTITLE), "[COLOR %s]Would you like to include a [COLOR %s]Backgrounds[/COLOR] folder?[/COLOR]" % (COLOR2, COLOR1)):
				fn = DIALOG.browse(0, 'Select location of backgrounds', 'files', '', True, False, HOMEPATH, False)
				if not fn == HOMEPATH:
					for base, dirs, files in os.walk(fn):
						...
						for file in files:
							try:
								...
								zipf.write(fn2, fn2[len(HOMEPATH):], zipfile.ZIP_DEFLATED)
							except Exception, e:
								log("[Back Up] Type = '%s': Unable to backup %s" % (type, file), xbmc.LOGNOTICE)
...
def restoreLocal(type):
	...
	if not os.path.exists(USERDATAPATH): os.makedirs(USERDATAPATH)
	if not os.path.exists(ADDOND): os.makedirs(ADDOND)
	if type == "gui": loc = USERDATAPATH
	elif type == "addondata": 
		loc = ADDOND
	else : loc = HOMEPATH
...
def Grab_Log(file=False, old=False):
	finalfile   = 0
	logfilepath = os.listdir(LOGPATH)
	logsfound   = []

	for item in logfilepath:
		if old == True and item.endswith('.old.log'): logsfound.append(os.path.join(LOGPATH, item))
		elif old == False and item.endswith('.log') and not item.endswith('.old.log'): logsfound.append(os.path.join(LOGPATH, item))
	...
	if os.path.exists(PACKAGESPATH):
		try:
			for root, dirs, files in os.walk(PACKAGESPATH):
				...
				file_count += len(files)
				if file_count > 0:
					size = convertSize(getSize(PACKAGESPATH))
					if over: yes=1
					else: yes=DIALOG.yesno("[COLOR %s]Delete Package Files[/COLOR]" % COLOR2, "[COLOR %s]%s[/COLOR] files found / [COLOR %s]%s[/COLOR] in size." % (COLOR1, str(file_count), COLOR1, size), "Do you want to delete them?", nolabel='[B]Don\'t Clear[/B]',yeslabel='[B]Clear Packages[/B]')
					if yes:
...
def clearThumb(type=None):
	...
	folder = convertSize(getSize(THUMBSPATH))
	if not type == None: choice = 1
	else: choice = DIALOG.yesno("[COLOR=blue]FTG Maintenance[/COLOR]",'[COLOR %s] The thumbnail folder Has exceeded the size of [COLOR red]%s MB[/COLOR] ' % (COLOR4, size), '[COLOR %s]Would you like to delete the [COLOR red]%s[/COLOR] of them?' % (COLOR4, folder), "They will repopulate on the next startup[/COLOR]", nolabel='[B]Don\'t Delete[/B]', yeslabel='[B]Delete Thumbs[/B]')
	if choice == 1:
		...
		try:
			for root, dirs, files in os.walk(PACKAGESPATH):
				...
				if file_count > 0:
					size = convertSize(getSize(PACKAGESPATH))
					if over: yes=1
					else: yes=DIALOG.yesno("[COLOR=blue]FTG Maintenance[/COLOR]",'[COLOR %s]The packages folder Has exceeded the size of [COLOR red]%s MB[/COLOR] ' % (COLOR4, filesize), "[COLOR %s]%s[/COLOR] files found / [COLOR %s]%s[/COLOR] in size." % (COLOR1, str(file_count), COLOR3, size), "Do you want to delete them?", nolabel='[B]Don\'t Clear[/B]',yeslabel='[B]Clear Packages[/B]')
					if yes:
						for f in files: os.unlink(os.path.join(root, f))
						for d in dirs: shutil.rmtree(os.path.join(root, d))
						LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE),'[COLOR %s]Clear Packages: Success![/COLOR]' % COLOR2)
...
def clearPackagesStartup():
	...
	if os.path.exists(PACKAGESPATH):
		pack = os.listdir(PACKAGESPATH)
		pack.sort(key=lambda f: os.path.getmtime(os.path.join(PACKAGESPATH, f)))
		try:
			for item in pack:
				file = os.path.join(PACKAGESPATH, item)
...
def RESET():
	log("Reset Kodi: Platform[%s]" % str(platform()), xbmc.LOGNOTICE)
...
def oldThumbs():
	...
	textexe.close()
	for image in images:
		path = os.path.join(THUMBSPATH, image)
		try:
			os.remove(path)
		except:
			pass
	finish = getSize(THUMBSPATH)
...
def RESTOREFAV():
	if os.path.exists(FAVfile):
		...
		if choice == 0:
			...
			xbmc.sleep(5)
			DP.close()
			DIALOG.ok(ADDONTITLE,'[COLOR=red]COMPLETE[/COLOR]', 'Your favorites are Restored.', '')
...
def BACKUPFAV():
	...
	if os.path.exists(FAVOURITES):
		...
		if choice == 0:
			...
			DIALOG.ok(ADDONTITLE,'[COLOR=red]COMPLETE[/COLOR]', 'Your favorites are Backed up.', '')
...
def DELFAV():
	if os.path.exists(FAVfile):
		...
		if choice == 0:
			...
			DIALOG.ok(ADDONTITLE,'[COLOR=red]COMPLETE[/COLOR]', 'Backed up deleted.', '')
	else: LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), '[COLOR %s]No Backups to remove![/COLOR]' % COLOR2)