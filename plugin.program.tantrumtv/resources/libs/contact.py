...
def credits(msg='', TxtColor='0xFFFFFFFF', Font='font12', BorderWidth=10):
    ...
    url = base_info.workingURL(CREDITSFILE)
    if url == True:
			link  = base_info.OPEN_URL(CREDITSFILE).replace('\r','').replace('\t','')
			id, msg = link.split('|||')
    ...
    else: base_info.loga("[Credits Notification] URL(%s): %s" % (CREDITSFILE, url))
####################################################################### 