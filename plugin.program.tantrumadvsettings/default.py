...
fanart = xbmc.translatePath(os.path.join('special://home/addons/plugin.program.tantrumadvsettings', 'fanart.jpg'))
iconart = xbmc.translatePath(os.path.join('special://home/addons/plugin.program.tantrumadvsettings', 'icon.png'))
SETTINGS_PATH = os.path.join(xbmc.translatePath('special://profile/addon_data/plugin.program.tantrumadvsettings'), 'settings.xml')
...
def MENU(name):
    ...
    addDirPlayable("Write XML File to temporary location", 'url',500, '','Play it safe. Write your advanced settings to userdata/plugin.program.tantrumadvsettings/XML_FILES/ directory',editfile,'')
...
def removexmlfile(name):
    if os.path.exists(ACTIVESETTINGSFILE):
        ...
        notification('Tantrum Advanced Settings', 'advancedsettings.xml removed', '4000', iconart)
...
def resetsettings(name):
    ...
    try:
        ...
        notification('Tantrum Advanced Settings', 'No changes exist', '4000', iconart)
        return
    for id in all_ids:
        if id != 'viewtype':
            ADDON.setSetting(id, value="DISABLED")
    notification('Tantrum Advanced Settings', 'All settings reset', '4000', iconart)
...
def networkmenu(name):
    ...
    addDir("Cache", 'url',303, '','blank','Network cache settings')
    name = "root"
...
def interface(name):
    name = "root"
    dirlist =  ['restrictcapsmask<>dialog<>Windows only. A bitmask to prevent xbmc from using detected texture capabilities of the GPU. This helps work around buggy hardware/drivers\n1: prevent the use of compressed textures (DXT1, DXT3, DXT5) 2: prevent the use of non-power-of-two dimensions for textures 4: prevent the use of compressed textures with non-power-of-two dimensions.<>["1","2","4","DISABLED"]',
    'gui$algorithmdirtyregions<>dialog<>Enable dirty-region processing. Dirty regions are any parts of the screen that have changed since the last frame. By not re-rendering what has not changed, big speed gains can be seen. Because all GPUs work differently, only Mode 3 is guaranteed to be safe for everyone\n0: Off-The entire viewport is always rendered\n1:    Union-All dirty regions are grouped into the smallest possible rectangle. This is typically the fastest mode for slower GPUs due to only making one pass.\n2: Cost reduction-Each dirty region is presented separately, in as many passes as there are regions\n3: Whole Screen-The entire screen is rendered if there are any dirty regions. This is a safe default for drivers that clear buffer contents (manifests as blinking or vibrating images).<>["0","1","2","3","DISABLED"]',
...
def cachebase(name):
    name = name.lower()
    dirlist =  ['memorysize<>text<>Number of bytes used for buffering streams ahead in memory XBMC will not buffer ahead more than this. WARNING: for the bytes set here, XBMC will consume 3x the amount of RAM. When set to 0 the cache will be written to disk instead of RAM, as of v12 Frodo \nRemember, 1MB = 1,048,576 bytes<>',
    ...
    'readfactor<>text<>This factor determines the max readrate in terms of readfactor * avg bitrate of a video file.This can help on bad connections to keep the cache filled. It will also greatly speed up buffering. Default value 1.0<>']
    for d in dirlist:
        splitd=d.split('<>')
        description = splitd[2]
        options = splitd[3]
...
def write_xml(name,dir_path):
    ...
    for s in settings:
        try:
            ...
            pass
    if count > 0:
        write_to_file(dir_path, '<!-- Created using Tantrum Advanced Settings addon -->\n<advancedsettings>\n', False)
        settinglist = ['video&lt;&gt;','audio&lt;&gt;','network&lt;&gt;','cache&lt;&gt;','edl&lt;&gt;','pvr&lt;&gt;','epg&lt;&gt;','samba&lt;&gt;','videolibrary&lt;&gt;','videodatabase&lt;&gt;','musicdatabase&lt;&gt;','musiclibrary&lt;&gt;','karaoke&lt;&gt;','pathsubstitution&lt;&gt;','masterlock&lt;&gt;']
    ...
    notification('Tantrum Advanced Settings', 'File created', '4000', iconart)
    xbmc.executebuiltin("Container.Refresh")
